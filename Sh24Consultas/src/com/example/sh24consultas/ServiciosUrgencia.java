    ...
    public ServiciosUrgencia ( java.math.BigDecimal pIdSms )  {
		...
		try{
	        ...
	        cStmt.setObject (2, "select rgenvio, cdasiste, "+
	        		" (select REPLACE(VALOR,'%texto%',( select fc_retornar_texto_link( exuah_mensajes_enviar.cdasiste ) from dual )  ) from exuah_paramproc where cdproceso = 'ENVIO_SMS_URG' AND CDPARAM = 'TEXTO_MSJE_URL') TEXTO " +
